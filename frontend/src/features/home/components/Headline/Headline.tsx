...
function Headline() {
  ...
  : HeadlineNews?.map ((article: any) => (
        <div className=" w-full  h-full flex flex-col justify-center items-center gap-2 "  onClick={() => handleNavigation(article?.slug)}>
          {(article?.headline?.display_type === "image" || article?.headline?.display_type === "mixed" || article?.headline?.display_type === "title_only") && 
          <>{(article?.headline?.display_type === "title_only" || article?.headline?.display_type === "mixed") && 
         ...
         <div className="flex w-[25%] h-[1/4] gap-5 items-center text-[rgb(var(--color-public-newsText-rgb)/0.6)]">
            <p className="flex items-center gap-1">
              <img src={article?.author?.image} className="h-7 w-7 rounded-full object-fill border" />
             { article?.author?.name}
            </p>
            <p className="flex items-center gap-1">
              <Clock size={20} />
           ...
           {(article?.headline?.display_type === "image" || article?.headline?.display_type === "mixed") &&<div className="relative w-full h-[50%] rounded-lg  cursor-pointer overflow-hidden">
              <img
