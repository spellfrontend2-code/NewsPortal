    ...
    <div className="w-full flex flex-col h-full">
      ...
      {isLoading || advertisementsLoading ? (
        ...
        <div className="flex flex-col lg:flex-row w-full h-[95%] gap-6">
          <div className={`flex flex-col lg:flex-row ${hasSidebarAds ? "lg:w-4/5" : "w-full"} gap-6`}>
            {firstArticle && (
            ...
            <div className="lg:w-1/5 w-full lg:sticky lg:top-20 self-start">
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                {sidebarAds.map((ad: any, index: number) => (
                  <div
                    key={ad.id ?? index}
                    className="w-full max-w-[320px] mx-auto aspect-[300/250] overflow-hidden rounded-md border border-[var(--color-public-border-light)] shadow-sm bg-white"
                  >
