import { useArticlesHooks } from "@/features/articles/hooks/useArticles";
import { useState } from "react";
import { useParams } from "react-router-dom";
import NewsListSkeleton from "./NewsListSkeleton";
import NewsList from "@/pages/Public/News/NewsList";
import { Newspaper } from "lucide-react";

function TagsBasedNewsListPage() {
  const color = "transparent";
  const { slug } = useParams();
  const articleHook = useArticlesHooks();
  const defaultPageSize = 13;

  const [pagination, setPagination] = useState({
    pageIndex: 0,
    pageSize: defaultPageSize,
  });

  const { data: allArticles, isLoading } =
    articleHook.useFetchPublicArticlesByTags({
      page: pagination?.pageIndex + 1,
      per_page: pagination?.pageSize,
      slug: slug,
    });
  const items = allArticles?.data ?? [];

  // Find category name from first article item
  const firstArticleItem = items.find(
    (item: any) => !item.type || item.type === "article",
  );
  const firstArticle = firstArticleItem?.data || firstArticleItem;
  const tagObj = firstArticle?.tags?.find((c: any) => c.slug === slug);
  const name = tagObj?.name || firstArticle?.tags?.name || slug;

  if (!isLoading && items.length === 0 && slug) return null;

  return (
    <div
      className={`relative w-full py-6`}
    >
      {isLoading ? (
        <NewsListSkeleton show={"all"} color={color} />
      ) : items.length > 0 ? (
        <NewsList
          articles={items}
          page_headline={name}
          slug={slug}
          pagination={pagination}
          setPagination={setPagination}
          lastPage={allArticles?.pagination?.last_page}
          show={"all"}
        />
      ) : (
        <div className="flex flex-col items-center justify-center py-12 px-4">
          <div className="flex items-center justify-center w-16 h-16 mb-4 rounded-full bg-slate-100">
            <Newspaper className="w-8 h-8 text-slate-400" />
          </div>
          <h3 className="text-lg font-semibold text-slate-700">
            No news available
          </h3>
          <p className="mt-1 text-sm text-slate-500 text-center max-w-sm">
            There are currently no news articles available in this tag.
            Please check back later.
          </p>
        </div>
      )}
    </div>
  );
}

export default TagsBasedNewsListPage;
