...
function NewsContent({ Data,commentRef }: any) {
  ...
  return (
    <div className="h-full overflow-y-auto flex flex-col lg:flex-row w-full gap-8">
      {/* Main Content Area */}
      <div className={`${advertisementData?.sidebar?.length > 0 ? "lg:w-3/4 w-full" : "w-full"} flex flex-col gap-6`}>
        {/* Media Block */}
        <div className="w-full rounded-2xl overflow-hidden shadow-md border border-slate-105 bg-slate-50">
        ...
        <div
          className=" text-justify prose prose-slate max-w-none text-slate-800 leading-relaxed text-lg 
            ...
            [&_h2]:text-3xl [&_h2]:font-bold [&_h2]:mt-10 [&_h2]:mb-4 [&_h2]:text-slate-900 [&_h2]:font-serif
            [&_h3]:text-2xl [&_h3]:font-semibold [&_h3]:mt-8 [&_h3]:mb-3 [&_h3]:text-slate-900 [&_h3]:font-serif
            ...
            [&_blockquote]:text-slate-700 [&_blockquote]:font-serif [&_blockquote]:text-xl [&_blockquote]:my-8
            [&_a]:text-[var(--color-public-newsText)] [&_a]:underline hover:[&_a]:opacity-80 "
        >
          <HtmlParser content={articleData?.content || ""} ad={advertisementData?.middle} />
        </div>
        ...
        <div className="my-8 w-full flex justify-center">
          <div className="flex flex-col md:flex-row gap-6 items-center justify-between bg-slate-50 border border-slate-200/60 rounded-3xl p-6 md:p-8 w-full shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-3">
              {/* Like Button */}
              <button
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border font-medium text-sm transition-all duration-200 cursor-pointer ${articleData?.user_interactions?.has_liked === true
                ...
                onClick={handleLike}
              >
                <ThumbsUp size={16} className={articleData?.user_interactions?.has_liked === true ? "fill-emerald-600" : ""} />
                <span>Like</span>
              ...
              {/* Dislike Button */}
              <button
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border font-medium text-sm transition-all duration-200 cursor-pointer ${articleData?.user_interactions?.has_dislike === true
                  ? "bg-rose-50 border-rose-200 text-rose-700 shadow-sm"
                  : "bg-white border-slate-200 hover:border-rose-200 hover:bg-rose-50/30 text-slate-655 hover:text-rose-700"
                  }`}
                onClick={handleDislike}
              >
                <ThumbsDown size={16} className={articleData?.user_interactions?.has_dislike === true ? "fill-rose-600" : ""} />
                <span>Dislike</span>
                <span className="h-4 w-px bg-slate-200 mx-1"></span>
      ...
      {advertisementData?.sidebar?.length > 0 && (
        <div className="lg:w-1/4 w-full flex flex-col gap-6">
          {advertisementData.sidebar.map((ad: any, index: number) => (
            <div key={ad.id ?? index} className="h-[160px] w-full overflow-hidden rounded-2xl border border-slate-200/60 shadow-sm">
